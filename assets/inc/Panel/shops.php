  ...
  <body>
    ...
    <div>
      <div class="full">
        ...
        <h6 class="heading-hr"><i class="icon-file"></i> Insert new Shops</h6>
                </ul>
          </div>
        <form action="add-shop.php" method="POST">
    ...
    <div>
      ...
      <div class="full">
        <div class="block-title">
        <h6 class="heading-hr"><i class="icon-file"></i> Search Shops</h6>
                </ul>
                </div>
          ...
          <div class="input-group">
            ...
            <span class="input-group-btn">
              <button type="submit" class="btn btn-default"><i class="fa fa-search text-info"></i></button>
                ...
                <hr>
                <div class="title"><strong>Shops</strong></div>
      ...
      if(!(isset($_POST['search-term']))){
        ...
        $sql = "SELECT * FROM `meh_items_shops` ORDER BY `id` DESC LIMIT ".$inicio.", ".$_BS['bypage'];
        $query = $db->prepare($sql);
        $query ->execute();
        ...
        foreach ($resultado as $row) {
          ...
          echo "
            <tr>
              ...
              <td>$name</td>
              <td>
              <a href='add-shop.php?edit=$id'><i class='fa fa-pencil-square-o text-info'></i></a>
              </td>
            </tr>
          ";
        }
        ...
        $busca_it = $db->prepare("SELECT * FROM meh_items_shops WHERE ((`Name` LIKE '%".$busca_prot."%') OR ('%".$busca_prot."%'))");
        $busca_it ->execute();
        $conta_it = count($busca_it->fetchAll( PDO::FETCH_BOTH ) );
        if($conta_it > 0){
           $busca_it = $db->prepare("SELECT * FROM meh_items WHERE ((`Name` LIKE '%".$busca_prot."%') OR ('%".$busca_prot."%'))");
        $busca_it ->execute();
