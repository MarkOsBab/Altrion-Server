...
if(isset($_GET['email']) && !empty($_GET['email']) AND isset($_GET['hash']) && !empty($_GET['hash'])){
    ...
    $db=getDB();  
    $search = $db->prepare("SELECT * FROM meh_users WHERE email='".$email."' AND hash='".$hash."' AND active='0'"); 
    $search->execute();
    $match = $search->fetchColumn(); 

    if($match > 0){
        // We have a match, activate the account
        $stmt = $db->prepare("UPDATE meh_users SET active='1' WHERE email='".$email."' AND hash='".$hash."' AND active='0'");
        $stmt->execute();
        echo '<div class="w3-container">
                  <div class="pt-2 pb-2 pl-2 pr-2 mt-3 text-center">
                    <h3>Hello, welcome to Altrion!</h3>
                    <h4 class="mt-5 mb-5">Your account has been activated, you can now <b><a class="button1 w3-hover-text-black w3-hover-yellow" style="text-decoration: none;" target="_blank" href="/play"><span>PLAY!</span></a></b></h4>
        ...
        echo '<div class="w3-container">
                  <div class="pt-2 pb-2 pl-2 pr-2 mt-3 text-center">
                    <h3 class="w3-text-red"><b>Your account has already been actived.</b></h3>                   
                  </div>
                </div>';
    }
